				      ...
				      struct apple_dart_master_cfg *cfg)
{
	...
	dma_addr_t dma_max = cfg->dma_max;
	int ret = 0;
	int i, j;
	...
	if (dart->locked) {
		...
		if (dart->hw->tcr_4level && dart->ias > 36) {
			if (readl(dart->regs + DART_TCR(dart, sid)) & dart->hw->tcr_4level) {
				...
				pgtbl_cfg.ias = 36;
				if (cfg->dma_min == 0 && dma_max == DMA_BIT_MASK(dart->ias)) {
					dma_max = DMA_BIT_MASK(pgtbl_cfg.ias);
				} else if ((cfg->dma_min ^ dma_max) & ~DMA_BIT_MASK(36)) {
					dev_err(dart->dev,
			       ...
			       const struct of_phandle_args *args)
{
	...
	if (args->args_count == 5) {
		...
		if (!(IS_ALIGNED(dma_min, dart->pgsize) || !IS_ALIGNED(size, dart->pgsize))) {
			dev_err(dev, "Unaligned DMA window %pad..%pad for page size 0x%x\n",
				&dma_min, &size, dart->pgsize);
			return -EINVAL;
		}
	...
	if (!cfg) {
		...
		cfg->locked = false;
		/* Will be merged with other DARTs to the to fully overlapping range. */
		cfg->dma_min = dma_min;
		cfg->dma_max = dma_max;
	} else {
...
static bool apple_dart_capable(struct device *dev, enum iommu_cap cap)
{
	switch (cap) {
